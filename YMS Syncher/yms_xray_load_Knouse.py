...
for row in rows:
    ...
    if building_id in ['06', '14']:
        ...
        print(f"Executing API_YMS_Inbound_LookupV2 with parameters: custid={custid}, po_number={po_number}")    
        # Call the stored procedure using the correct database
        cursor.execute("{CALL API_YMS_Inbound_LookupV2(?, ?)}", custid, po_number)
        print(f"executing API_YMS_Inbound_LookupV2")
        ...
        json_output = result
        print(f"Stored Procedure Result - Ref Number: {row.Ref_1}, Appt Number: {row.Appt_Num}, Facility Code: {building_id}, JSON Output: {json_output}")
