...
for row in rows:
    ...
    building_id = "0614"
    cust_id = "KNOUSE_FOODS"

    print (f"{building_id}")

    # Check if building_id is either '0614'
    if building_id in ['0614']:
        ...
        print(f"PO Number: {po_number}, Confirmation Number: {confirmation_number}, Building ID: {building_id},  Customer ID: {cust_id}")

        # Call the stored procedure using the correct database
        cursor.execute("{CALL API_YMS_Inbound_XRAY_Lookup_Knouse (?, ?, ?, ?)}", 
                       po_number, confirmation_number, building_id, cust_id)
        result = cursor.fetchone()

        if not result or not result[3]:
            skipped_records.append((po_number, confirmation_number, building_id , cust_id))
