...
SELECT 
  ...
  AND Trailer_Status = 'loaded'
  AND load_type IN ('Shuttle', 'bulk', 'Floor Load')
  AND ref_1 IS NOT NULL 
  AND Facility IN ('Building 72', 'Building 38')  -- Modified line to include both buildings
  ORDER BY ref_1 DESC
"""
...
for row in rows:
    ...
    if building_id in ['38', '72']:
        ...
        cursor.execute("{CALL API_YMS_XRAY_GenerateJsonPayload_brawhi (?, ?, ?)}", 
